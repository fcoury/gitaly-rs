...
fn main() -> ExitCode {
    ...
    let binary = env::var_os("GITALY_SSH_BIN").unwrap_or_else(|| OsString::from("ssh"));
    match Command::new(&binary).args(&args).status() {
        Ok(status) => exit_from_status(status),
        Err(source) => {
            eprintln!(
                "gitaly-ssh: failed to execute '{}': {source}",
...
fn print_help() {
    println!(
        "Usage: gitaly-ssh [ssh-args...]\n\
         \n\
         Execute `ssh` with passthrough arguments. Override binary with\n\
         `GITALY_SSH_BIN`."
    );
}
